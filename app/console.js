...
// ---------------------------------------------------------------------------------------
function login () {
	// Load the login form definition
	var form = [
		{	"name" : "loginForm",
	...
	// Show the login form
	ui.formEdit('loginForm', {"username":'', "password":''});
}
...
//		ui.messageBox('CON008', [port]);
	});

	// Errors
	socket.on('error', function(err) {
		ui.messageBox('CON009', [port]);
	});
...
// ---------------------------------------------------------------------------------------
// Start up function called by 'body onload'
// ---------------------------------------------------------------------------------------
function onStart () {
    // Load the validation checks
//    ui.validation(define_checks());
    ui.validation();

    // Initialise the form post-processing script
    ui.postProcess(form_post_process);
